...
SELECT ss.shippingid,
	   ...
	   CASE
		   WHEN ss.shipping_end_fact_datetime > s.shipping_plan_datetime THEN DATE_PART('day', AGE(ss.shipping_end_fact_datetime, s.shipping_plan_datetime))
		   ELSE 0
	   END AS delay_day_at_shipping,
	   si.payment_amount,
	   ROUND(si.payment_amount * (scr.shipping_country_base_rate + sa.agreement_rate + st.shipping_transfer_rate), 2) AS vat,
	   ROUND(si.payment_amount * sa.agreement_commission, 2) AS profit
